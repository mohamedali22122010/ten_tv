...
<div class="nav-tabs-custom">
	...
	@foreach(config('laravel-translatable.languages') as $language=>$name)
		<div id="tab-{{$name}}" class="tab-pane clearfix"  data-tab-index="{{$data_tab_index}}">
			<div class="form-group">
				<label>{{trans('backend.Video Title')}}</label>
				<input class="form-control" name="title[{{$language}}]" type="text" data-parsley-required="true" value="{{old('title.'.$language,$video->getTranslation('title',$language))}}" placeholder="" >
			</div>
	...
	<div id="tab-general" class="tab-pane"  data-tab-index="{{$data_tab_index}}">
		...
		<div class="form-group ">
            ...
            <select class="form-control" data-parsley-required="true" name="program_id">
                ...
                @if($programs && !empty($programs))
                    ...
                    <option value="{{$id}}" @if($video->program_id == $id) selected @endif >{{$value}}</option>
                    @endforeach
                @endif
            </select>
        </div>
		...
		<div class="form-group">
			<label>{{trans('backend.Video Link')}}</label>
			<input class="form-control" name="link" type="text" value="{{old('link',$video->link)}}" placeholder="" >
		</div>

        <div class="form-group">
                <label class="col-md-3"><input class="minimal" name="is_home" type="checkbox" @if($video->is_home) checked @endif> {{ trans('backend.Set At Home Page') }} </label>           
        </div>
